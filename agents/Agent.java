...
public class Agent {
	...
	while(countLAYER < max){
		...
		if(i != this.index && !isTie(as, i, layer) && (addUtil[layer][i] - currentUtil) > maxGain && (i != as.NUM_PLAYERS-1 || as.preshock)){ 
	...
	if (bestAdd[1] > 6 && time > as.timeOfShock)
	    System.out.println(bestAdd[1]);//TODO
	return bestAdd; 
    }
	...
	for(layer=0;layer<max;layer++)
		...
		if(numTies(as,layer) > 0 && searchSize > 0)
			...
			for(i = 0; i < as.NUM_PLAYERS; i++)
			    {
				// if neighbor
				if(i != this.index && as.coplayerMatrix[layer][this.index][i]&&searchSize > 0 && (i != as.NUM_PLAYERS-1 || as.preshock))
				    {
					...
					//check if we can make triangle benefits
						...
						for(j=0; j < as.NUM_PLAYERS; j++)
							...
							if(j != i && j != this.index && as.coplayerMatrix[layer][i][j] && searchSize > 0 && (i != as.NUM_PLAYERS-1 || as.preshock))
							    {
	...
	while(countLAYER < max){
	    while(countPLAYER < as.NUM_PLAYERS){
		if(i != this.index && isTie(as, i, layer) && (lossUtil[layer][i] - currentUtil) > maxGain && (i != as.NUM_PLAYERS-1 || as.preshock)){ 
	...
	while(addCount < max){//start at random layer			
		...
		if(i != this.index && !isTie(as, as.agentList[i], addLayer) && (i != as.NUM_PLAYERS-1 || as.preshock)){ //if no tie exists 					
		    Agent addAgent = as.agentList[i];					
		    while(dropCount < max){ //loop over potential deletes
			while(countDROPPLAYER < as.NUM_PLAYERS){
			    if(j != this.index && j != i && isTie(as, as.agentList[j], dropLayer) && (i != as.NUM_PLAYERS-1 || as.preshock)){ //if tie exists
	...
	long time = as.schedule.getSteps();
	if (bestAddDrop[0][1] > 6 && time > as.timeOfShock)
	    System.out.println(bestAddDrop[0][1]);//TODO
	return bestAddDrop; 
    }
